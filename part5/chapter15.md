...
```
    ...
    LCRC仅保护链路上的TLP。一旦入口端口的数据链路层检查LCRC，
    它就将其从数据包中移除，因为将在出口端口计算新的LCRC。
    这意味着数据包在交换机内部时不受保护。这就是拥有ECRC的目的。
```
...
| 书中错误类型 | Linux宏定义 |
|-------------|------------|
| Poisoned TLP | PCI_ERR_UNC_POISON_TLP |
| Completion Timeout | PCI_ERR_UNC_COMP_TIME |
| Unsupported Request | PCI_ERR_UNC_UNSUP |
| ECRC Error | PCI_ERR_UNC_ECRC |
| Replay Timer Timeout | PCI_ERR_COR_REP_TIMER |
