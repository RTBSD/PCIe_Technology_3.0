...
![简单PLL框图](../images/图2-5.简单PLL框图.png)
...
*原文图示：配置头*
       ...
       设备 ID    供应商 ID            设备 ID    供应商 ID
       状态寄存器  命令寄存器            状态寄存器  命令寄存器
           类代码      版本 ID              类代码      版本 ID
       BIST  头类型  延迟定时器  缓存行大小   BIST  头类型  延迟定时器  缓存行大小
       
       基地址寄存器 0                  基地址寄存器 0
       基地址寄存器 1                  基地址寄存器 1
       基地址寄存器 2                  次要延迟定时器  从属总线号  次要总线号  主总线号
       基地址寄存器 3                  次要状态  IO 限制  IO 基址
       基地址寄存器 4                  内存限制    内存基址
       基地址寄存器 5                  可预取内存限制  可预取内存基址
       CardBus CIS 指针               可预取基址上限 32 位
       子系统 ID    子系统供应商 ID      可预取限制上限 32 位
       扩展 ROM 基址                   IO 限制上限 16 位  IO 基址上限 16 位
       保留    能力指针                保留    能力指针
       保留                           扩展 ROM 基址
       Max_Lat Min_Gnt 中断引脚 中断线   桥控制  中断引脚  中断线
```
...
```
                    图 2-12：PCI Express 设备层

![TLP组装](../images/图2-12.TLP组装.png)
...
![PCIe中断](../images/图2-16.PCIe中断.jpg)

*原文图示：PCIe中断*


                    TLP 核心部分的信息来自软件层/设备核心
...
![错误报告](../images/图2-17.错误报告.png)

*原文图示：错误报告*



                    TLP 核心部分的信息发送到软件层/设备核心
...
```
     ...
     Ack/Nak 等。
  (1)                     数据                数据                          (4)
 DLLP 核心 CRC          链路层              链路层                  DLLP 核心 CRC

(2)           (2)                                               (3)           (3)
SDP DLLP 核心 CRC END    物理层              物理层           SDP DLLP 核心 CRC END
                          (RX)    (TX)       (RX)    (TX)
```
...
```
                 ...
                 事务层              事务层
                     发送                 接收
                                   数据链路层
              链路数据包     DLLP            DLLP       链路数据包
                                ACK /          ACK /
     序列号      TLP    LCRC      NAK            NAK   序列号   TLP    LCRC
...
```
                    图 2-26：数据链路层的 TLP 和 DLLP 结构


                                        事务层数据包 (TLP)
     序列 ID          头              数据载荷              ECRC   LCRC
     ...
     DLLP 的基本形式也由上图显示，包括一个 4 字节的 DLLP 类型字段，可能包含一些其他信息，以及一个 2 字节的 CRC。
```

```
                    图 2-27：带 Ack/Nak 协议的非发布事务
...
```
                    图 2-28：物理层的 TLP 和 DLLP 结构


                                        事务层数据包 (TLP)
     起始  序列号        头              数据载荷              ECRC    LCRC 结束
     1B      2B        3-4 DW            0-1024 DW             1DW   1DW   1B



                                        DLLP
                          起始 DLLP 类型     杂项        CRC 结束
                           1B         1DW               2B   1B
```
...
```
     ...
     请求者                                                                   完成者
发送内存读取请求                    软件层                    接收内存读取请求



    事务层数据包 (TLP)                                              事务层数据包 (TLP)
            头              ECRC                                        头              ECRC
...
```
     ...
     请求者                                                                   完成者
接收带数据的完成                    软件层                    发送带数据的完成



    事务层数据包 (TLP)                                              事务层数据包 (TLP)
头        数据载荷       ECRC                                        头        数据载荷       ECRC
